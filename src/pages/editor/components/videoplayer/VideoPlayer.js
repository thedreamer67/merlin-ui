...
const VideoPlayer = (props) => {
	...
	// Sync videoplayer with timeline scrollbar on scroll
	useEffect(() => {
		function onScroll() {
			if (props.seeking) {
	...
	// Sync videoplayer with timeline scrollbar on scroll using shift + mousewheel
	useEffect(() => {
	...
	// Get current frame the videoplayer is at on spell drop on videoplayer
	useEffect(() => {
	...
	// Generate captions on backend using audio file
	...
	async function initCaption() {
		...
		if (generatingCaption === 200) {
			...
			props.setSubtitles(captionRetrieved);
	...
	useEffect(() => {
		(async function getVid() {
			...
			if (project.timelinevideo_ids.length === 2) {
				if (props.isFinal) {
					props.setVideoURL(demo_final);
				} else {
					props.setVideoURL(demo_bgreplaced);
				}
			}
		})();
	...
	return (
		<div className='video-component'>
			<div
				id='video-player'
				className={
					props.isSpellDragActive && props.timelineVids.length > 0
